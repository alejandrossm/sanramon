...
- La recuperacion de contrasena por correo debe aplicar a usuarios internos activos con contrasena utilizable. Los socios no tienen contrasena propia.
- Al crear socios, el formulario no debe solicitar contrasena inicial; la cuenta tecnica debe quedar sin password utilizable.
- Las lecciones aprendidas reutilizables quedan documentadas en `docs/LECCIONES_APRENDIDAS.md`.

## Pendientes

- Sprint 3: HU-12 completada con landing publica del Proyecto San Ramon como punto de inicio, navbar con acceso al sistema y consulta de asistencia, galeria con imagenes `f01`, `f02`, `f03`, footer con contacto/redes, y consulta iniciada por RUT pero protegida por codigo temporal enviado al correo registrado, aceptacion versionada de privacidad y sesion limitada. El resultado muestra estado del socio, historial anual, resumen anual y totales generales. El calculo de proxima reunion queda disponible como servicio, pero no se muestra en la vista publica. HU-13 completada con reportes descargables desde los listados de asistencia y socios en `xlsx`, `csv` y `pdf`, filtrados por ano/estado y con exportacion del conjunto completo. HU-14 completada con auditoria en `auditoria.log` para desactivacion/activacion de usuarios, cancelacion y borrado de reuniones, borrado de usuarios/socios y respaldo de base de datos.
- Cumplimiento de proteccion de datos: la memoria especifica de Ley N° 21.719 se mantiene en `docs/MEMORIA_CUMPLIMIENTO_LEY_21719.md`; debe actualizarse junto con los controles, proveedores, plazos y politica publica.
- Carga historica de asistencia: las reuniones `HISTORICA` muestran accion de carga, se ofrece plantilla XLSX exportable a CSV, y la importacion acepta separador `;` o `,` procesando cada registro de forma transaccional.
- Seguridad previa a produccion: HTTPS/cookies, proteccion anti fuerza bruta, bloqueo de comandos demo y validacion global de identidades ya estan implementados. Queda activar y comprobar `Force HTTPS` en PythonAnywhere, validar el entorno objetivo y cerrar las decisiones organizativas de cumplimiento.
